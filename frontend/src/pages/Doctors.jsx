        ...
        <div className='w-full grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-4 gap-y-6'>
          {
            filterDoc.slice(0, 10).map((item, index) => (
              <div onClick={() => navigate(`/appointment/${item._id}`)} className='border border-black-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-2.5 transitioin-all duration-500 ' key={index}>
